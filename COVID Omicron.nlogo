...
to reset-turtles
  create-turtles population-sample
...
to go
  ask turtles [
    get-older
    move
    if status = "Infected" [ recover-or-die ]
...
to update-global-variables
  if count turtles > 0
    [ set %infected (count turtles with [ status = "Infected" ] / count turtles) * 100
      set %immune (count turtles with [ status = "Recovered" ] / count turtles) * 100 ]
end

to update-display
  ask turtles
    [ if shape != people-shape [ set shape people-shape ]
      set color ifelse-value status = "Infected" [ red ] [ ifelse-value status = "Recovered" [ grey ] [ green ] ] ]
end

to get-older
  set age age + 1
  if age > lifespan [ die ]
  if status = "Recovered" [ set remaining-immunity remaining-immunity - 5 ]
  if status = "Infected" [ set sick-time sick-time + 1 ]
end
...
to startup
  reset-constants ;; so that carrying-capacity can be used as upper bound of population-sample slider
end
@#$#@#$#@
GRAPHICS-WINDOW
540
